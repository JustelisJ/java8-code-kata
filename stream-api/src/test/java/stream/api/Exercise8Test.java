...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream
                ...
                .map(Item::getName)
                .filter(s -> !itemListOnSale.contains(s))
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = customer -> {
            Integer sumOfWanted = customer.getWantToBuy().stream()
                    .map(item -> {
                        Optional<Integer> lowestPrice = onSale.stream()
                                .filter(saleItem -> saleItem.getName().equals(item.getName()))
                                .map(Item::getPrice)
                                .min(Integer::compare);
                        return lowestPrice.orElseGet(() -> {return 0;});
                    })
                    .reduce(0, Integer::sum);
            return customer.getBudget() >= sumOfWanted;
        };
